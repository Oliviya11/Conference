...
public class SetAnnouncementServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       List<Conference> filteredIterable= new ArrayList<Conference>();
       for (Conference confrernce: iterable){
    	   if (confrernce.getSeatsAvailable()<5)
    		   filteredIterable.add(confrernce);
       }
        ...
        for (Conference conference : filteredIterable) {
        ...
        if (conferenceNames.size() > 0) {
            ...
            String announcementString = announcementStringBuilder.toString();
            memcacheService.put(announcementKey, announcementString);
